...
import type { Simple } from '../../weather/meteo/src/types.ts'
...
Deno.test('has correct headers', async () => {
	const response = await fetch('http://0.0.0.0:8787/?provider=auto')
...
Deno.test('Geolocation fails without query', async () => {
	const response = await fetch('http://0.0.0.0:8787/?provider=auto&geo=true')
	expect(response.status).not.toBe(200)
})

Deno.test('Geolocation returns locations', async () => {
	const response = await fetch('http://0.0.0.0:8787/?provider=auto&geo=true&query=Paris')
	expect(response.status).toBe(200)
